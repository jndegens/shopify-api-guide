...
const bundles = groupDefinitions.map((group) => ({ ...group, scopes: STANDARD_SCOPES.filter((scope) => groupForScope(scope) === group.id) }));
...
export default function ShopifyApiPage() {
  ...
  const scopes = useMemo(() => {
    const standard = bundles.filter((bundle) => selected.includes(bundle.id)).flatMap((bundle) => bundle.scopes);
    return [...standard, ...(dangerEnabled ? DANGER_SCOPES : [])].join(',');
  }, [selected, dangerEnabled]);
    ...
    <section className="wizard-shell" id="stappen">
      <article className="wizard-step"><div className="step-number">01</div><div className="step-body"><div className="step-title"><div><p className="eyebrow dark">EENMALIG IN SHOPIFY</p><h2>Maak de app klaar</h2><p className="step-intro">Werk deze zes stappen van boven naar beneden af. Je blijft hiervoor in Shopify.</p></div><HelpButton topic="app" open={openHelp} /></div>
        <ol className="setup-steps">
          <li><span className="setup-index">1</span><div className="setup-copy"><b>Open het Dev Dashboard</b><p>Klik op de knop hieronder en log in met hetzelfde Shopify-account als van je store. Je hebt geen apart developer-account nodig.</p><a className="dashboard-link" href={DEV_DASHBOARD_URL} target="_blank" rel="noopener noreferrer">Open Shopify Dev Dashboard <span aria-hidden="true">↗</span></a><p className="fallback-route">Lukt de directe link niet? Ga dan in Shopify naar <strong>Instellingen → Apps en verkoopkanalen → Apps ontwikkelen → Apps bouwen in Dev Dashboard</strong>.</p></div></li>
          <li><span className="setup-index">2</span><div className="setup-copy"><b>Maak een app en versie aan</b><p>Klik op <strong>App aanmaken</strong>, vul een herkenbare naam in en klik daarna op <strong>Versie aanmaken</strong>.</p></div></li>
          <li><span className="setup-index">3</span><div className="setup-copy"><b>Plak de App-URL</b><p>Laat het veld <strong>App-URL</strong> niet leeg. Kopieer deze URL en plak hem in Shopify.</p><div className="step-value"><code>{APP_URL}</code><CopyButton value={APP_URL} label="Kopieer App-URL" /></div></div></li>
          <li><span className="setup-index">4</span><div className="setup-copy"><b>Voeg de omleidings-URL toe</b><p>Ga naar <strong>Toegestane omleidings-URL&apos;s</strong>, klik op toevoegen en plak deze URL.</p><div className="step-value"><code>{REDIRECT_URL}</code><CopyButton value={REDIRECT_URL} label="Kopieer omleidings-URL" /></div></div></li>
          <li><span className="setup-index">5</span><div className="setup-copy"><b>Kies en kopieer de API-rechten</b><p>De acht werkgebieden geven veilige leesrechten. Wil je dat de koppeling ook kan wijzigen, verwijderen en financiële of klantdata kan openen? Zet dan één keer de <strong>Danger zone</strong> aan.</p>
            <div className="scope-picker"><div className="picker-head"><div><strong>Kies de API-rechten</strong><p>Alle acht werkgebieden staan aan. Gevoelige rechten blijven apart en standaard uitgeschakeld.</p></div><div className="picker-actions"><HelpButton topic="scopes" open={openHelp} /><button type="button" onClick={() => { setSelected(selected.length === bundles.length ? [] : allBundleIds); setAuthorizationUrl(''); }}>{selected.length === bundles.length ? 'Wis alles' : 'Selecteer alle 8'}</button></div></div><div className="bundle-grid">{bundles.map((bundle) => <button type="button" aria-pressed={selected.includes(bundle.id)} className={selected.includes(bundle.id) ? 'bundle active' : 'bundle'} key={bundle.id} onClick={() => toggle(bundle.id)}><i aria-hidden="true">{selected.includes(bundle.id) ? '✓' : '+'}</i><span><strong>{bundle.title}</strong><small>{bundle.hint} · {bundle.scopes.length} veilige scopes</small></span></button>)}</div>
              <section className={dangerEnabled ? 'danger-zone active' : 'danger-zone'} aria-labelledby="danger-title"><div className="danger-copy"><span>DANGER ZONE</span><h3 id="danger-title">Alles kunnen wijzigen</h3><p>Voegt alle {DANGER_SCOPES.length} schrijf-, klantdata-, order- en financiële Admin API-rechten toe. Gebruik dit alleen in jouw eigen teststore.</p><div className="danger-tags"><i>Klantdata</i><i>Orders & geld</i><i>Wijzigen & verwijderen</i></div></div><button type="button" aria-pressed={dangerEnabled} onClick={() => { setDangerEnabled((current) => !current); setAuthorizationUrl(''); }}><b>{dangerEnabled ? '✓ Danger zone actief' : 'Danger zone inschakelen'}</b><small>{dangerEnabled ? 'Klik om gevoelige scopes te verwijderen' : 'Voegt alle gevoelige scopes toe'}</small></button></section>
              <div className="scope-output"><span>{scopeCount} SHOPIFY ADMIN API-SCOPES {dangerEnabled ? '· VOLLEDIGE TESTTOEGANG' : '· VEILIGE MODUS'}</span><code>{scopes || 'Selecteer minimaal één groep'}</code><CopyButton value={scopes} label={`Kopieer ${scopeCount} scopes`} /></div>
              <details className="scope-notes"><summary>Waarom staan sommige Shopify-scopes hier niet tussen?</summary><div><p><strong>Shopify-goedkeuring nodig:</strong> deze {SHOPIFY_APPROVAL_SCOPES.length} scopes worden niet gekopieerd, omdat Shopify ze voor deze studentenapp afwijst zonder aparte toestemming.</p><code>{SHOPIFY_APPROVAL_SCOPES.join(', ')}</code><p><strong>Andere API:</strong> <code>customer_*</code>, <code>unauthenticated_*</code> en <code>shop_app:oauth</code> uit Shopify&apos;s volledige keuzelijst gebruiken Customer Account-, Storefront- of Shop API-tokens. Ze horen niet in deze Admin OAuth-link.</p></div></details>
            </div>
          </div></li>
          <li className="publish-step"><span className="setup-index">6</span><div className="setup-copy"><b>Geef de versie vrij</b><p>Klik rechtsboven in Shopify op <strong>Vrijgeven</strong> en bevestig. Pas daarna werkt de installatielink in stap 2.</p><div className="publish-callout"><span>LAATSTE CONTROLE</span><strong>Zie je de status Vrijgegeven?</strong><p>Dan is je app klaar en kun je hieronder verder met stap 2.</p></div></div></li>
        </ol>
      </div></article>
