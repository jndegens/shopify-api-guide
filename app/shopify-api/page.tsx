...
- Nadat de bereiken zijn gekozen, laat je me rechtsboven op Vrijgeven klikken. Laat de optionele versienaam automatisch staan, laat de beschrijving leeg en laat me Vrijgeven bevestigen. Ga pas daarna naar App installeren.
...
Start nu meteen met het eerste complete actieblok. Neem alle eenvoudige kliks mee en stop pas bij het eerste moment waarop ik iets moet invullen, kiezen, controleren of aan jou moet teruggeven.`, [normalizedShop, oauthEndpoint, scopeLine]);
  ...
  return <main>
    ...
    <section className="scope-section" id="rechten"><div className="scope-intro"><p className="kicker">Hoort bij stap 03</p><h2>Acht taken. Eén klik per keuze.</h2><p>Kies wat je AI-tool moet kunnen. Elke keuze zet automatisch alle bijbehorende Shopify-scopes klaar.</p><div className="scope-summary"><strong>{selectedGroupCount}/8</strong><span>volledige taakbundels gekozen</span><small>{scopes.length} scopes{ sensitiveCount > 0 ? ` · ${sensitiveCount} gevoelig` : ''}</small></div><button className="select-all" onClick={() => setScopes(scopes.length === allScopes.length ? [] : allScopes)}>{scopes.length === allScopes.length ? 'Alles deselecteren' : 'Alle acht taken selecteren'}</button><p className="fineprint">“Alles” is bewust geen standaard. Shopify kan voor beschermde klantdata of financiële scopes extra goedkeuring eisen.</p></div><div className="scope-builder task-scope-builder">
      ...
      <div className="copy-box"><span>Plak deze regel bij API access → Bereiken selecteren</span><code>{scopeLine || 'Selecteer minimaal één taak'}</code><CopyButton value={scopeLine}>Kopieer Shopify-bereiken</CopyButton></div>
      <article className="publish-card" aria-labelledby="publish-title"><div className="publish-copy"><span>Stap 03 afronden</span><h3 id="publish-title">Vergeet niet: geef de versie vrij</h3><p>Je scopes zijn pas klaar voor installatie nadat je de versie hebt gepubliceerd. Shopify noemt dit <strong>Vrijgeven</strong>.</p><ol><li><b>1</b><span>Klik rechtsboven op <strong>Vrijgeven</strong>.</span></li><li><b>2</b><span>Laat de versienaam automatisch staan en de beschrijving leeg.</span></li><li><b>3</b><span>Bevestig nog één keer met <strong>Vrijgeven</strong>.</span></li></ol></div><div className="publish-dialog" aria-label="Voorbeeld van het Shopify-venster Versie vrijgeven"><header><div><small>AI Store Connector</small><strong>Versie vrijgeven</strong></div><button type="button" tabIndex={-1} aria-hidden="true">×</button></header><p>Deze versie wordt actief en kan daarna op je store worden geïnstalleerd.</p><label>Versienaam <small>Optioneel</small><span>Automatisch ingevuld</span></label><label>Beschrijving <small>Optioneel</small><span className="empty-field">Leeg laten</span></label><footer><button type="button" tabIndex={-1}>Annuleren</button><button type="button" tabIndex={-1}>Vrijgeven</button></footer></div></article>
    </div></section>
