    ...
    <section className="wizard-shell" id="stappen">
      <article className="wizard-step"><div className="step-number">01</div><div className="step-body"><div className="step-title"><div><p className="eyebrow dark">EENMALIG IN SHOPIFY</p><h2>Maak de app klaar</h2><p className="step-intro">Werk deze zes stappen van boven naar beneden af. Je blijft hiervoor in Shopify.</p></div><HelpButton topic="app" open={openHelp} /></div>
        <ol className="setup-steps">
          ...
          <li><span className="setup-index">5</span><div className="setup-copy"><b>Kies en kopieer de API-rechten</b><p>Laat alle acht groepen geselecteerd als je de volledige testtoegang wilt. Klik daarna op de gele kopieerknop en plak de regel bij <strong>API-toegang → Bereiken</strong>.</p>
            <div className="scope-picker"><div className="picker-head"><div><strong>Kies de API-rechten</strong><p>Alle 8 groepen staan standaard aan. De zeven rechten die Shopify voor deze studentenroute afwijst, zijn al uit de lijst gehaald.</p></div><div className="picker-actions"><HelpButton topic="scopes" open={openHelp} /><button type="button" onClick={() => { setSelected(selected.length === bundles.length ? [] : allBundleIds); setAuthorizationUrl(''); }}>{selected.length === bundles.length ? 'Wis alles' : 'Selecteer alle 8'}</button></div></div><div className="bundle-grid">{bundles.map((bundle) => <button type="button" aria-pressed={selected.includes(bundle.id)} className={selected.includes(bundle.id) ? 'bundle active' : 'bundle'} key={bundle.id} onClick={() => toggle(bundle.id)}><i aria-hidden="true">{selected.includes(bundle.id) ? '✓' : '+'}</i><span><strong>{bundle.title}</strong><small>{bundle.hint} · {bundle.scopes.length} scopes</small></span></button>)}</div><div className="scope-output"><span>{scopeCount} SHOPIFY ADMIN API-SCOPES</span><code>{scopes || 'Selecteer minimaal één groep'}</code><CopyButton value={scopes} label={`Kopieer ${scopeCount} scopes`} /></div><p className="scope-warning"><strong>Testomgeving:</strong> sommige overige rechten kunnen extra Shopify-goedkeuring of winkelinstellingen vereisen. De gekopieerde lijst bevat niet de scopes uit de foutmelding.</p></div>
          </div></li>
