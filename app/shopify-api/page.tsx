...
export default function ShopifyApiGuide() {
  ...
  const supportPrompt = useMemo(() => `Ik volg een tutorial om voor mijn eigen Shopify-store een blijvende API-koppeling in te stellen. Begeleid mij vanaf de stap waar ik vastloop.
...
Vraag nu bij welke stap ik ben en wat ik precies zie.`, [normalizedShop]);
  ...
  return <main>
    ...
    <section className="hero" id="top"><div className="eyebrow">Shopify × jouw AI-tool</div><h1>Maak je Shopify API-koppeling.<br /><em>Stap voor stap.</em></h1><p className="lede">Deze tutorial laat zien hoe je voor je eigen Shopify-store een blijvende API-koppeling instelt. Je maakt een app aan, geeft de juiste rechten en bewaart Client ID en Client secret veilig. De AI-tool kan daarmee steeds een nieuw 24-uurs access token aanvragen.</p></section>

    <section className="ai-help" aria-labelledby="ai-help-title"><div className="ai-help-copy"><span className="passport-label">Hulp nodig?</span><h2 id="ai-help-title">Laat je AI-tool veilig meekijken.</h2><p>Plak eerst je Shopify Admin-URL. De pagina haalt het permanente winkeldomein eruit en zet het automatisch in de hulpprompt.</p><label className="prompt-store" htmlFor="shop-domain"><span>Shopify Admin-URL of winkeldomein</span><input id="shop-domain" value={shopInput} placeholder="https://admin.shopify.com/store/jouw-winkel/apps/…" aria-invalid={shopInput.length > 0 && !normalizedShop} aria-describedby="shop-result" onInput={(event) => setShopInput(event.currentTarget.value)} /><span id="shop-result" className={normalizedShop ? 'shop-valid' : 'shop-hint'} aria-live="polite">{normalizedShop ? `✓ ${normalizedShop} staat in de prompt` : shopInput ? shopResult.error : 'Je kunt de prompt ook zonder store kopiëren.'}</span></label><CopyButton value={supportPrompt}>Kopieer hulpprompt</CopyButton></div><pre><code>{supportPrompt}</code></pre></section>

    <section className="guide" id="stappen"><header className="section-head"><p className="kicker">Van nul naar koppeling</p><h2>Acht duidelijke stappen.</h2><p>Volg de schermen in dezelfde volgorde als in Shopify.</p></header>
    ...
    <section className="guide continue-guide">
      <article className="step reverse"><div className="step-copy"><span className="step-number">05</span><p className="kicker">Vrijgeven</p><h3>Release de appversie</h3><p>Plak de gemaakte regel in <strong>API access → Bereiken selecteren</strong>. Controleer de App-URL en scopes en klik op <strong>Vrijgeven</strong>. De automatisch voorgestelde versienaam is prima; de beschrijving mag leeg blijven.</p><div className="note">Later een scope toevoegen? Maak en release een nieuwe versie. De store moet nieuwe toegang opnieuw goedkeuren.</div></div><div className="release-card"><span>CONTROLE VOOR RELEASE</span><label><input type="checkbox" readOnly checked /> Standaard App-URL ingevuld</label><label><input type="checkbox" readOnly checked /> Benodigde scopes gekozen</label><label><input type="checkbox" readOnly checked /> Geen secret gedeeld</label><button tabIndex={-1}>Vrijgeven</button></div></article>
      <article className="step"><div className="step-copy"><span className="step-number">06</span><p className="kicker">Installeren</p><h3>Kies je store en installeer</h3><p>Klik linksboven op de appnaam en kies <strong>App installeren</strong>. Selecteer je store, bekijk de gevraagde rechten en klik <strong>Installeren</strong>.</p><div className="note warning"><b>Meerdere stores?</b> Laat de actieve store vóór elke wijziging zichtbaar bevestigen. Binnen een project dat aantoonbaar aan één store is gekoppeld hoeft dat niet telkens opnieuw.</div></div><MiniScreen kind="install" /></article>
      <article className="step reverse"><div className="step-copy"><span className="step-number">07</span><p className="kicker">Credentials</p><h3>Sla Client ID en Client secret veilig op</h3><p>Open de <strong>appinstellingen</strong>. Vul Client ID en Client secret uitsluitend in het beveiligde secret-formulier van je AI-tool in. De backend vraagt daarmee server-side een access token aan dat na 24 uur verloopt.</p><ul className="never-list"><li>Nooit in een AI-chat, Slack, e-mail of een ticket.</li><li>Nooit in frontendcode, screenshots, Git of logs.</li><li>Nooit credentials of tokens tussen stores hergebruiken.</li></ul></div><div className="credential-stack"><article><span className="safe">Identificatie</span><h4>Client ID</h4><p>Identificeert de app en hoort bij deze appconfiguratie.</p></article><article><span className="danger">Geheim</span><h4>Client secret</h4><p>Alleen server-side in een secret manager, nooit in chat of browseropslag.</p></article><article><span className="danger">24 uur</span><h4>Access token</h4><p>Niet als permanente sleutel behandelen; na verloop opnieuw server-side aanvragen.</p></article></div></article>
      <article className="step"><div className="step-copy"><span className="step-number">08</span><p className="kicker">Controleren</p><h3>Begin met een onschuldige lezing</h3><p>Laat de AI-tool eerst de shopnaam en toegekende scopes opvragen. Controleer het teruggegeven <code>myshopify.com</code>-domein. Pas daarna mag er geschreven worden.</p><div className="success-box"><b>Koppeling geslaagd wanneer:</b><span>✓ installatie actief</span><span>✓ juiste winkel teruggegeven</span><span>✓ verwachte scopes aanwezig</span><span>✓ read-only test slaagt</span></div></div><div className="verify-card"><span>READ-ONLY CHECK</span><code>query {'{'} shop {'{'} name myshopifyDomain {'}'} {'}'}</code><div><i /> Store gevonden</div><div><i /> Domein komt overeen</div><div><i /> Scopes gecontroleerd</div></div></article>
    </section>
