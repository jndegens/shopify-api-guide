...
export default function ShopifyApiPage() {
  ...
  const scopes = useMemo(
    () => bundles.filter((bundle) => selected.includes(bundle.id)).flatMap((bundle) => bundle.scopes).join(','),
    [selected],
  );
    ...
    <section className="wizard-shell" id="stappen">
      <article className="wizard-step"><div className="step-number">01</div><div className="step-body"><div className="step-title"><div><p className="eyebrow dark">EENMALIG IN SHOPIFY</p><h2>Maak de app klaar</h2><p className="step-intro">Werk deze zes stappen van boven naar beneden af. Je blijft hiervoor in Shopify.</p></div><HelpButton topic="app" open={openHelp} /></div>
        <ol className="setup-steps">
          ...
          <li><span className="setup-index">5</span><div className="setup-copy"><b>Kies de API-rechten</b><p>Kies wat de koppeling in Shopify mag doen. Alle acht werkgebieden staan standaard aan; klik op een categorie om die uit te zetten.</p>
            <div className="scope-picker"><div className="picker-head"><div><strong>Kies de API-rechten</strong><p>Elke gekozen categorie bevat alle geldige bijbehorende scopes. Kopieer daarna de regel naar <strong>API-toegang → Bereiken selecteren</strong> in Shopify.</p></div><div className="picker-actions"><HelpButton topic="scopes" open={openHelp} /><button type="button" onClick={() => { setSelected(selected.length === bundles.length ? [] : allBundleIds); setAuthorizationUrl(''); }}>{selected.length === bundles.length ? 'Wis alles' : 'Selecteer alle 8'}</button></div></div><div className="bundle-grid">{bundles.map((bundle) => <button type="button" aria-pressed={selected.includes(bundle.id)} className={selected.includes(bundle.id) ? 'bundle active' : 'bundle'} key={bundle.id} onClick={() => toggle(bundle.id)}><i aria-hidden="true">{selected.includes(bundle.id) ? '✓' : '+'}</i><span><strong>{bundle.title}</strong><small>{bundle.hint} · {bundle.scopes.length} scopes</small></span></button>)}</div>
              <div className="scope-output"><span>{scopeCount} GELDIGE SHOPIFY ADMIN API-SCOPES · {selected.length} VAN 8 CATEGORIEËN GESELECTEERD</span><code>{scopes || 'Selecteer minimaal één categorie'}</code><CopyButton value={scopes} label={`Kopieer ${scopeCount} scopes`} /></div>
      ...
      <article className="wizard-step final-step"><div className="step-number">04</div><div className="step-body"><div className="step-title"><div><p className="eyebrow dark">AUTOMATISCH TERUG</p><h2>Vul je secret in en kopieer de token</h2></div><HelpButton topic="token" open={openHelp} /></div><div className="single-action"><b>Na Installeren kom je automatisch terug.</b><p>Vul op de terugkeerpagina je <strong>Client secret</strong> in en klik op <strong>Token maken</strong>. Kopieer de token direct naar de beveiligde secret-invoer van je project.</p></div><aside className="privacy-note"><span aria-hidden="true">✓</span><div><strong>Je API-key wordt niet door deze website opgeslagen</strong><p>Je Client secret en access token worden niet bewaard of gelogd. De secret wordt één keer via HTTPS gebruikt; de token staat daarna alleen tijdelijk op jouw scherm. Kopieer hem direct naar je eigen beveiligde secretopslag.</p></div></aside><aside className="security"><strong>Nooit delen of hergebruiken</strong><p>Zet een Client secret of access token nooit in chat, screenshots, e-mail, Git of logs. Gebruik je meerdere stores? Bewaar per token altijd het bijbehorende <code>.myshopify.com</code>-domein.</p></aside></div></article>
    </section>
